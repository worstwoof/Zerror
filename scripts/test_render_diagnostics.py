...
class RenderDiagnosticsTest(unittest.TestCase):
    ...
    def test_physics_manim_uses_blackboard_layout(self) -> None:
        ...
        self.assertIn("row.arrange(RIGHT", source)
        self.assertIn("RoundedRectangle(width=13.15", source)
        self.assertIn("group.to_edge(UP", source)
        self.assertIn("derivation_left = 1.15", source)
        self.assertIn("derivation_top = 2.58", source)
        ...
        self.assertIn("force_arrow_centered = Arrow(block_final_center, block_final_center + RIGHT * 1.18", source)
        self.assertIn("f_on_b = Arrow(block_final_center, block_final_center + RIGHT * 0.82", source)
