...
- 第一阶段只承诺“题目不丢”：保存 OCR 文本和基础卡片，返回 `partial_result`。
- 第二阶段承诺“答案质量”：使用高质量文本模型补全答案、公式推导、错因和复习建议，完成后返回 `result`。
- 如果第二阶段失败，任务保持 `partial_success`，前端应展示基础结果并提供“重新生成详解”。
- 公式清洗只做保守修复：`rac{a}{b}` / `frac{a}{b}` 可修成 `\frac{a}{b}`；`racmv_0eB` 这类边界不明的片段不自动猜测，避免生成错误答案。
- Manim 动画是增强内容，渲染失败不影响题目详解；错误细节保留在 diagnostics，前端展示友好提示即可。
