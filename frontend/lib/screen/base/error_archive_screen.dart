...
class _ErrorArchiveScreenState extends State<ErrorArchiveScreen> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            icon:
                const Icon(Icons.search_rounded, color: AppPalette.textPrimary),
            onPressed: () {},
          ),
        ],
      ),
  ...
  Widget _buildStatsHeader(AppStore store) {
    return Container(
      ...
      child: Row(
        ...
        children: [
          _buildStatItem(
              '累计收录', '${store.totalErrors}', AppPalette.textPrimary),
          Container(
            width: 1,
            height: 40,
          ...
          _buildStatItem(
              '待复习', '${store.pendingReviewCount}', AppPalette.moodBlue),
          Container(
            width: 1,
            height: 40,
            color: AppPalette.inkBlue.withOpacity(0.10),
          ...
          _buildStatItem(
              '已掌握', '${store.masteredCount}', AppPalette.textPrimary),
        ],
      ),
    );
  }
  ...
  Widget _buildStatItem(String label, String value, Color valueColor) {
    return Column(
      children: [
        ...
        Text(label,
            style:
                const TextStyle(color: AppPalette.textPrimary, fontSize: 12)),
      ],
    );
  }
  ...
  Widget _buildErrorCard(
      BuildContext context, AppStore store, ErrorRecord item) {
    return Container(
      ...
      child: Material(
        ...
        child: Ink(
          decoration: BoxDecoration(
            color: AppPalette.paper.withOpacity(0.95),
            borderRadius: BorderRadius.circular(24),
            border: Border.all(
              color: item.isMastered
                  ? AppPalette.matchaMist.withOpacity(0.3)
                      ...
                      IconButton(
                        ...
                        onPressed: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              content: Text(
                                  item.isFavorite ? '已取消收藏这道错题' : '已加入我的收藏'),
                      ...
                      ElevatedButton(
                        onPressed: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                                builder: (context) =>
                                    ErrorDetailScreen(errorId: item.id)),
                          );
                        },
                        style: ElevatedButton.styleFrom(
                          ...
                          side: item.isMastered
                              ? const BorderSide(color: AppPalette.matchaMist)
                              : BorderSide.none,
                          shape: RoundedRectangleBorder(
  ...
  Widget _subjectPill(String label) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: AppPalette.matchaMist.withOpacity(0.12),
